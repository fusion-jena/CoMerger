...
public class BinaryHolisticTestBalance {
...
//		runTimeVersion5();
	...
	private static void runVersion5() throws IOException, OWLOntologyCreationException, OWLOntologyStorageException {
		...
		String ResultPath = "C:\\Users\\Samira\\Desktop\\HolisticDataSet\\result_binary_balance.csv";
		...
		for (int i = startIndex; i <= currentNumberDataSet; i++) {
			if (i != 7 && i != 8) {
				...
				boolean done = true;
				while (done) {
					if (inputOnt.length % 2 != 0)
						newInputOnt.add(inputOnt[inputOnt.length - 1]);
					// CopyFileToNewFolder(inputOnt[inputOnt.length - 1],
					// UPLOAD_DIRECTORY);

					for (int j = 0; j < inputOnt.length - 1; j++) {
						String mappingFile = "";
						try {
							inputOntList = inputOnt[j] + ";" + inputOnt[j + 1];
						...
						// CopyFileToNewFolder(ontM.getOntName(),
						// UPLOAD_DIRECTORY);
						newInputOnt.add(ontM.getOntName());

						// check it is finished

					}

					inputOnt = newInputOnt.toArray(new String[newInputOnt.size()]);
					newInputOnt = new ArrayList<String>();

					if (inputOnt.length <= 1)
						done = false;
	...
	private static void runTimeVersion5()
		...
		String ResultPath = "C:\\Users\\Samira\\Desktop\\HolisticDataSet\\result_binary_balance_time.csv";
		GenerateOutput.createOutputHeader(ResultPath);

		int currentNumberDataSet = 12;
		int startIndex = 12; // in 1 & 2, n=2
		int time = 3;
		String[] address = new String[currentNumberDataSet + 1];
		String baseAddress = "C:\\Users\\Samira\\Desktop\\HolisticDataSet\\";
		for (int j = startIndex; j <= currentNumberDataSet; j++)
	...
	private static void CopyFileToNewFolder(String ontName, String Path) {
		File src = new File(ontName);
