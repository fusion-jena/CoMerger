...
public class HBlockRefinePacket {
			...
			while (iterCl.hasNext()) {
				...
				if (!flag) {
					...
					if (father != null) {
						...
						if (refFather != null)
							father = refFather;
						int clusterId = BlockShareFunc.findClusterOfClass(father, ontM);
						if (clusterId != -1) {
						...
						if (child != null) {
							...
							if (refChild != null)
								child = refChild;
							int clusterId = BlockShareFunc.findClusterOfClass(child, ontM);
							if (clusterId != -1) {
				...
				if (!flag) {
					...
					if (domainSet.size() != 0) {
						...
						if (domain instanceof OWLClassImpl) {
							...
							if (refDomain != null)
								domain = refDomain;
							int clusterId = BlockShareFunc.findClusterOfClass(domain.asOWLClass(), ontM);
							if (clusterId != -1) {
								manager = ontM.getClusters().get(clusterId).getManager();
								factory = manager.getOWLDataFactory();
								clusterOnt = ontM.getClusters().get(clusterId).getOntology();
					...
					if (rangeSet.size() != 0) {
						...
						if (range instanceof OWLClassImpl) {
							...
							if (refRange != null)
								range = refRange;
							int clusterId = BlockShareFunc.findClusterOfClass(range.asOWLClass(), ontM);
							if (clusterId != -1) {
								manager = ontM.getClusters().get(clusterId).getManager();
								factory = manager.getOWLDataFactory();
								clusterOnt = ontM.getClusters().get(clusterId).getOntology();
				...
				if (!flag) {
					...
					if (rangeIter != null && rangeIter.hasNext()) {
						...
						if (range instanceof OWLClassImpl) {
							...
							int clusterId = BlockShareFunc.findClusterOfClass(((OWLClassImpl) range).asOWLClass(),
									ontM);
							if (clusterId != -1) {
								manager = ontM.getClusters().get(clusterId).getManager();
								factory = manager.getOWLDataFactory();
							...
							// Set<OWLDatatype> dType =
							// range.getDatatypesInSignature(); //
							// ((OWLDatatypeImpl)range).getDataRangeType()
							// OWLAxiom newAxiom1 =
			...
			while (iterIn.hasNext()) {
				...
				if (!flag) {
					...
					if (cIter != null && cIter.hasNext()) {
						...
						if (refC != null)
							c = refC;
						int clusterId = BlockShareFunc.findClusterOfClass(c, ontM);
						if (clusterId != -1) {
							manager = ontM.getClusters().get(clusterId).getManager();
							factory = manager.getOWLDataFactory();
							clusterOnt = ontM.getClusters().get(clusterId).getOntology();
	...
	public static HModel StructurePreservation(HModel ontM) {
		...
		for (int i = 0; i < ontM.getInputOntNumber(); i++) {
			...
			while (iterOi.hasNext()) {
				...
				int clusterId = BlockShareFunc.findClusterOfAxiom(ax, ontM);

				// now we know in which clusters, we have this axiom
				if (clusterId != -1) {
					OWLAxiom eqAx = ontM.getEqAxioms().get(ax);
					clusterId = BlockShareFunc.findClusterOfAxiom(eqAx, ontM);
				}
				...
				if (clusterId != -1) {
					...
					while (elm.hasNext()) {
						...
						clusterId = BlockShareFunc.findClusterOfClass(el, ontM);
						if (clusterId != -1)
							break;
					}
					if (clusterId != -1) {
			...
			while (iter.hasNext()) {
				...
				if (!exist) {
					...
					if (!cm.getIRI().toString().equals(OWL.Thing.getURI().toString())) {
						int clusterId = BlockShareFunc.findClusterOfClass(cm, ontM);
						manager = ontM.getClusters().get(clusterId).getManager();
						clusterOnt = ontM.getClusters().get(clusterId).getOntology();
	...
	private static void deleteExtraObjectProperty(HModel ontM) {
		...
		for (int k = 0; k < ontM.getClusters().size(); k++) {
			...
			while (iter.hasNext()) {
				...
				if (!exist) {
					// delete cm from Om
					int clusterId = BlockShareFunc.findClusterOfPro(cm, ontM);
					manager = ontM.getClusters().get(clusterId).getManager();
					clusterOnt = ontM.getClusters().get(clusterId).getOntology();
					remover = new OWLEntityRemover(manager, Collections.singleton(clusterOnt));
					cm.accept(remover);
	...
	private static void deleteExtraDataProperty(HModel ontM) {
		...
		for (int k = 0; k < ontM.getClusters().size(); k++) {
			...
			while (iter.hasNext()) {
				...
				if (!exist) {
					// delete cm from Om
					int clusterId = BlockShareFunc.findClusterOfDro(cm, ontM);
					manager = ontM.getClusters().get(clusterId).getManager();
					clusterOnt = ontM.getClusters().get(clusterId).getOntology();
					remover = new OWLEntityRemover(manager, Collections.singleton(clusterOnt));
					cm.accept(remover);
	...
	private static void deleteExtraInstances(HModel ontM) {
		...
		for (int k = 0; k < ontM.getClusters().size(); k++) {
			...
			while (iter.hasNext()) {
				...
				if (!exist) {
					// delete cm from Om
					int clusterId = BlockShareFunc.findClusterOfIndiv(cm, ontM);
					manager = ontM.getClusters().get(clusterId).getManager();
					clusterOnt = ontM.getClusters().get(clusterId).getOntology();
					remover = new OWLEntityRemover(manager, Collections.singleton(clusterOnt));
					cm.accept(remover);
...
//			}
			...
			for (int j = 0; j < OmEqDataPro.size(); j++) {
				...
				// Set<OWLClassExpression> dataDomainRef =
				// Dpro.getDomains(clusterOnt);
