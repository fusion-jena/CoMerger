...
{
    private ArrayList<MappingOwl> collection = null;

    public AlignmentOwl()
    {
        collection = new ArrayList<MappingOwl>();
    }

    public void addMapping(MappingOwl map)
    {
        collection.add(map);
    }

    public MappingOwl getMapping(int index)
    {
    ...
    {
    	MappingOwl map = contains(left, right);
        if (map == null) {
            return 0;
        } else {
    ...
    public boolean removeMapping(Node left, Node right)
    {
        for (int i = 0, n = size(); i < n; i++) {
        	MappingOwl map = collection.get(i);
    ...
    public MappingOwl contains(Node left, Node right)
    {
        for (int i = 0, n = size(); i < n; i++) {
        	MappingOwl map = collection.get(i);
            if (map.getEntity1().equals(left) && 
    ...
    public AlignmentOwl cut(double threshold)
    {
        for (int i = 0; i < size(); i++) {
        	MappingOwl align = collection.get(i);
